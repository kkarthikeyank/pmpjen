...
Call log:
  - waiting for getByRole('textbox', { name: 'Enter a doctor\'s name' })
    ...
    at ProviderPage.fillDoctorName (/home/karthi/pmp/src/pages/ProviderPage.js:90:34)
    at /home/karthi/pmp/src/tests/claimsearch.spec.js:201:20
```
...
>  90 |     await this.doctorNameTextbox.click();
  ...
  158 | //     await this.page.getByText('Return to previous page').click();
  159 | // }
  160 |
  161 | // async addProviderById(id) {
  162 | //     await this.page.locator(`[id="${id}"]`).getByRole('button', { name: 'Add to my directory' }).click();
  163 | // }
  164 |
  165 | // async checkCompareBox(id) {
  166 | //     await this.page.locator(`[id="${id}compare"]`).check();
  167 | //     await this.page.getByRole('checkbox', { name: 'Compare with other providers' }).check();
  168 | //     await this.page.getByRole('button', { name: /Compare providers/ }).click();
  169 | // }
  170 |
  171 | // async removeComparison() {
  172 | //     await this.page.getByText('Provider ID55641').click();
  173 | //     await this.page.getByText('Remove').nth(1).click();
  174 | // }
  175 |
  176 | async logout() {
  177 |   // Ensure the dropdown menu button is visible and click it
  178 |   await expect(this.logoutButton).toBeVisible();  // Ensure the menu button is visible
  179 |   await this.logoutButton.click();  // Open the dropdown menu
  180 |   
  181 |   // Ensure the logout button in the dropdown is visible before clicking
  182 |   await expect(this.logoutButtonProfile).toBeVisible();  // Ensure logout button is visible
  183 |   await this.logoutButtonProfile.click();  // Perform the logout action
  184 | }
  185 |
  186 |
  187 | }
