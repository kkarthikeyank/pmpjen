...
Call log:
  - waiting for locator('text=Thursday, May 1,').locator('text=1')

    at LabsPage.selectDate (/home/karthi/pmp/src/pages/LabsPage.js:131:47)
    at LabsPage.selectStartDate (/home/karthi/pmp/src/pages/LabsPage.js:138:16)
    at LabsPage.setCustomDateRange (/home/karthi/pmp/src/pages/LabsPage.js:149:16)
...
```ts
  ...
  129 |     // Use the 'text' selector to find the day in the label
  130 |     const labelLocator = this.dateLabel(dateLabel);
> 131 |     await labelLocator.locator(`text=${day}`).click();  // Locate the day text
      |                                               ^ Error: locator.click: Test timeout of 70000ms exceeded.
  132 |     await this.dateLabel('Select year').selectOption(year);
  133 |     await this.dateLabel('Select month').selectOption(month);
  134 |   }
  135 |
  136 |   // Set the start date
  137 |   async selectStartDate(day, month, year) {
  138 |     await this.selectDate('Thursday, May 1,', day, month, year);
  139 |   }
  140 |
  141 |   // Set the end date
  142 |   async selectEndDate(day, month, year) {
  143 |     await this.selectDate('Saturday, November 30,', day, month, year);
  144 |   }
  145 |
  146 |   // Set the custom date range (start and end dates)
  147 |   async setCustomDateRange(startDay, startMonth, startYear, endDay, endMonth, endYear) {
  148 |     await this.openCustomDateFilter();  // Calls the openDatePicker method from the same class
  149 |     await this.selectStartDate(startDay, startMonth, startYear);
  150 |     await this.selectEndDate(endDay, endMonth, endYear);
  151 |   }
  152 | }
  153 |
  154 |
