...
Call log:
  - waiting for locator('role=gridcell[name="15"][selected=false]').first()

    at LabsPage.selectDate (/home/karthi/pmp/src/pages/LabsPage.js:138:85)
    at LabsPage.selectDateRange (/home/karthi/pmp/src/pages/LabsPage.js:142:5)
...
```ts
  ...
  129 |     // Get all year/month dropdowns and calendar days
  130 |     const yearDropdowns = this.page.getByLabel('Select year');
  131 |     const monthDropdowns = this.page.getByLabel('Select month');
  132 |
  133 |     // Select year and month dynamically based on labelIndex (0 for start, 1 for end)
  134 |     await yearDropdowns.nth(labelIndex).selectOption(year);
  135 |     await monthDropdowns.nth(labelIndex).selectOption(month);
  136 |
  137 |     // Click the day - fallback to contains matching if ARIA label is not reliable
> 138 |     await this.page.locator(`role=gridcell[name="${day}"][selected=false]`).first().click();
      |                                                                                     ^ TimeoutError: locator.click: Timeout 10000ms exceeded.
  139 |   }
  140 |
  141 |   async selectDateRange(start, end) {
  142 |     await this.selectDate(0, start.year, start.month, start.day); // start
  143 |     await this.selectDate(1, end.year, end.month, end.day);       // end
  144 |   }
  145 | }
  146 |
  147 |
