...
- Location: /home/karthi/pmp/src/tests/claimspagegroup.spec.js:37:3
...
Call log:
  - waiting for locator('//button[@id=\'dropdownDateFilterButton\']')
    - locator resolved to <button type="button" aria-expanded="false" _ngcontent-upf-c276="" data-bs-toggle="dropdown" id="dropdownDateFilterButton" data-id="dropdownDateFilterButton" class="btn btn-outline-gray btn-block dropdown-toggle text-start w-100 h-70">…</button>
  ...
  - retrying click action
    ...
    at Function.filterClaimsAndSelectProviders (/home/karthi/pmp/src/pages/ClaimsPage.js:403:28)
    at /home/karthi/pmp/src/tests/claimspagegroup.spec.js:39:5
```
...
> 403 |     await dateFilterButton.click();
  ...
  406 |
  407 |   await expect(radioLocator).toBeVisible({ timeout: 5000 });
  408 |     await radioLocator.check();
  409 |
  410 |     await doneButton.click();
  411 |
  412 |     await filterResultsButton.click();
  413 |     console.log("[INFO] Filter panel opened.");
  414 |
  415 |     // Select providers
  416 |     const count = await providerBlocks.count();
  417 |
  418 |     for (let i = 0; i < count; i++) {
  419 |       const container = providerBlocks.nth(i);
  420 |       const nameElement = container.locator('div.me-2');
  421 |       const nameText = (await nameElement.innerText()).trim();
  422 |
  423 |       if (providerNames.includes(nameText)) {
  424 |         await container.locator('input[type="checkbox"]').check();
  425 |         console.log(`✔ Selected provider: ${nameText}`);
  426 |       } else {
  427 |         console.log(`✖ Skipped: ${nameText}`);
  428 |       }
  429 |     }
  430 |   }
  431 |
