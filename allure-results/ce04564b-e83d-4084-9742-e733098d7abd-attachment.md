...
Call log:
  - waiting for locator('//button[@id=\'applyFilterClaims\']')
    - locator resolved to <button disabled tabindex="0" aria-current="page" _ngcontent-hgq-c274="" id="applyFilterClaims" class="btn btn-primary" aria-label="applyFilters">…</button>
...
```ts
  ...
  145 |
  146 |     if (!isChecked) {
  147 |       await checkboxLocator.first().check();
