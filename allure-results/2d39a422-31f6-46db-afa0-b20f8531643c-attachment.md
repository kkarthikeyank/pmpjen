...
- textbox "Enter a doctor's name": test
- paragraph: Plan
...
>  82 |   await planOption.click();
  ...
  133 | }
  134 |
  135 |  async printSearchResults() {
  136 |     const count = await this.doctorNameResults.count();
  137 |
  138 |     if (count === 0) {
  139 |       console.log('[INFO] No doctor name displayed');
  140 |       return;
  141 |     }
  142 |
  143 |     for (let i = 0; i < count; i++) {
  144 |       const doctorName = await this.doctorNameResults.nth(i).textContent();
  145 |       console.log(`[DOCTOR ${i + 1}]: ${doctorName.trim()}`);
  146 |
  147 |       const plan = await this.planLocator.nth(i).textContent().catch(() => null);
  148 |       if (plan && plan.trim()) {
  149 |         console.log(`  └─ Plan: ${plan.trim()}`);
  150 |       } else {
  151 |         console.log(`  └─ No plan displayed under doctor`);
  152 |       }
  153 |     }
  154 |   }
  155 |   
