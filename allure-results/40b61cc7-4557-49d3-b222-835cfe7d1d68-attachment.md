...
```
Error: expect.toBeVisible: Unexpected token "=" while parsing css selector "table.claims-table, text=No claims found". Did you mean to CSS.escape it?
Call log:
  - expect.toBeVisible with timeout 30000ms
  - waiting for table.claims-table, text=No claims found

    at ClaimsPage.filterByCustomDateRangeAndPrintClaims (/home/karthi/pmp/src/pages/ClaimsPage.js:362:32)
...
```ts
  ...
  360 | const tableOrNoResults = this.page.locator('table.claims-table, text=No claims found');
  361 |
> 362 | await expect(tableOrNoResults).toBeVisible({ timeout: 30000 });
      |                                ^ Error: expect.toBeVisible: Unexpected token "=" while parsing css selector "table.claims-table, text=No claims found". Did you mean to CSS.escape it?
  363 |     // Get claims
  364 |     const claims = await this.claimNumberLocator.all();
  365 |     if (claims.length === 0) {
  366 |       console.log(`❌ No claims found for range ${fromDate} to ${toDate}`);
  367 |     } else {
  368 |       console.log(`✅ Claims found for range ${fromDate} to ${toDate}:`);
  369 |       for (let i = 0; i < claims.length; i++) {
  370 |         const claim = claims[i];
  371 |         const text = await claim.textContent();
  372 |         console.log(`→ Claim Number: ${text?.trim()}`);
  373 |
  374 |         // Open and return for each claim
  375 |         await this.openClaimAndReturn(i + 1);
  376 |       }  
  377 |     }
  378 |   }
  379 |
  380 |  async openClaimAndReturn(claimIndex) {
  381 |     if (claimIndex <= 0) {
  382 |         console.log(`❌ No claims available to open.`);
  383 |         return;
  384 |     }
  385 |
  386 |     console.log(`→ Opening claim ${claimIndex}...`);
  387 |
  388 |     const claimDetailButton = this.viewDetailsButton.nth(claimIndex - 1); // Use nth for 0-indexed array
  389 |     await claimDetailButton.click();
  390 |
  391 |     await this.page.waitForSelector('text=Claim Details', { timeout: 20000 });
  392 |     console.log(`✅ Claim ${claimIndex} details page opened.`);
  393 |
